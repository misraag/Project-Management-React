...
export default function YourProjects({listOfProjects, viewNewTask, ...props}) {
    ...
    return (
        <div {...props} className=" flex flex-col w-[30vw] bg-black p-10 gap-5 text-white">
            ...
            {listOfProjects.map((listItem) => {
                return <li ref={projectName} onClick={() => viewNewTask(listItem.title)} className="bg-red-300 my-4 hover:cursor-pointer">{listItem.title}</li>
            })}
            </ul>
